import {
  ...
  ClinicalRecordPatientNotFoundError,
} from "../../domain/clinical-record.errors";
...
export class ClinicalRecordApplicationService {
  ...
  async createClinicalRecord(
    command: CreateClinicalRecordCommand,
  ): Promise<CreateClinicalRecordResult> {
    ...
    const clinicalRecord =
      await this.dependencies.clinicalRecordPersistencePort.createClinicalRecord(command);
    return { clinicalRecord };
  }

  async getClinicalRecord(query: GetClinicalRecordQuery): Promise<GetClinicalRecordResult> {
    const clinicalRecord =
      await this.dependencies.clinicalRecordPersistencePort.findClinicalRecordById(query.id);
