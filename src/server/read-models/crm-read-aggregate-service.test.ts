...
import type { CrmFolioReadModel, PatientReadModel, TreatmentInterestReadModel, WorksheetReadModels } from "./worksheet-read-models";
...
describe("crm read aggregate service", () => {
  it("builds CRM aggregates without extending the Patient aggregate", () => {
    const result = buildCrmReadAggregatesFromReadModels(models({
      treatmentInterests: [interest({ treatmentInterestId: "TI-001" })],
      crmFolios: [folio({ crmFolioId: "CF-001" })],
    }));

    expect(result.crmAggregates).toEqual([{ patientId: "PAT-001", treatmentInterests: [expect.objectContaining({ treatmentInterestId: "TI-001" })], crmFolios: [expect.objectContaining({ crmFolioId: "CF-001" })] }]);
    expect(result.diagnostics).toMatchObject({ totalPatients: 1, totalTreatmentInterests: 1, totalCrmFolios: 1, patientsWithTreatmentInterests: 1, patientsWithCrmFolios: 1 });
  });
  ...
  it("keeps orphan CRM rows diagnostic instead of failing the read model", () => {
    ...
    expect(result.crmAggregates).toEqual([{ patientId: "PAT-001", treatmentInterests: [], crmFolios: [] }]);
  ...
  it("sorts CRM data by newest operational timestamp", () => {
    ...
    expect(result.crmAggregates[0]?.treatmentInterests.map((item) => item.treatmentInterestId)).toEqual(["TI-NEW", "TI-OLD"]);
    expect(result.crmAggregates[0]?.crmFolios.map((item) => item.crmFolioId)).toEqual(["CF-NEW", "CF-OLD"]);
  });
});
