import { readClinicalOutcomesForPatient, type ClinicalOutcomeReadDto } from "@/server/read-models/clinical-outcome-read-adapter";
import { readTreatmentPlansForPatient, type TreatmentPlanReadDto } from "@/server/read-models/treatment-plan-read-adapter";
import { readTreatmentStagesForPatient, type TreatmentStageReadDto } from "@/server/read-models/treatment-stage-read-adapter";
...
export function buildClinicalReadAggregatesFromReadModels(models: WorksheetReadModels): ClinicalReadAggregateResult {
  ...
  return {
    ...
    diagnostics: {
      ...
      incompleteTreatmentPlans: treatmentPlans.filter((plan) => !normalize(plan.treatmentPlanId) || !normalize(plan.planName)).length,
      incompleteTreatmentStages: treatmentStages.filter((stage) => !normalize(stage.treatmentStageId) || !normalize(stage.stageName)).length,
      incompleteClinicalOutcomes: clinicalOutcomes.filter((outcome) => !normalize(outcome.clinicalOutcomeId) || !normalize(outcome.outcomeType)).length,
    },
  };
}
