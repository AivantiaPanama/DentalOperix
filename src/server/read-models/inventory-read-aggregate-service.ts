import { readConsumables, type ConsumableReadDto } from "@/server/read-models/consumable-read-adapter";
import { readProducts, type ProductReadDto } from "@/server/read-models/product-read-adapter";
import { readStockLevels, type StockLevelReadDto } from "@/server/read-models/stock-level-read-adapter";
...
export function buildInventoryReadAggregateFromReadModels(models: WorksheetReadModels): InventoryReadAggregateResult {
  ...
  return {
    ...
    diagnostics: {
      ...
      incompleteProducts: products.filter((product) => !normalize(product.productId) || !normalize(product.productName)).length,
      incompleteConsumables: consumables.filter((consumable) => !normalize(consumable.consumableId) || !normalize(consumable.consumableName)).length,
      incompleteStockLevels: stockLevels.filter(
        (stockLevel) => !normalize(stockLevel.stockLevelId) || !normalize(stockLevel.productId) || !normalize(stockLevel.warehouseId),
      ).length,
      incompleteWarehouses: warehouses.filter((warehouse) => !normalize(warehouse.warehouseId) || !normalize(warehouse.warehouseName)).length,
    },
  };
}
