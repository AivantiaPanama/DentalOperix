...
class InMemoryPatientRepository implements PatientRepository {
  ...
  async createPatient(input: CreatePatientInput) { return createPatientEntity(input); }
  async findPatientById(id: string) { return this.patients.find((patient) => patient.id === id) ?? null; }
  async updatePatient(): Promise<Patient> { throw new Error("not needed"); }
  async searchPatientsByIdentity(search: PatientIdentitySearch) {
    return this.patients.filter((patient) =>
      ...
      patient.identifiers.some((identifier) => identifier.type === search.identifierType && identifier.normalizedValue === search.identifierValue),
    );
  }
}
...
describe("Patient identity resolution", () => {
  it("returns strong_match for normalized name and email", async () => {
    const existing = createPatientEntity({ displayName: "Ana Ruiz", source: "web", emails: [{ email: "ana@example.com" }] }, { id: "patient_1" });
    const service = new PatientIdentityResolutionService(new InMemoryPatientRepository([existing]));

    const result = await service.resolveBeforeCreate({ displayName: "Ana Ruiz", source: "web", emails: [{ email: "ANA@example.com" }] });
  ...
  it("returns ambiguous_match when same name lacks safe differentiator", async () => {
    const existing = createPatientEntity({ displayName: "Luis García", source: "admin", phones: [{ phone: "111" }] }, { id: "patient_2" });
    const service = new PatientIdentityResolutionService(new InMemoryPatientRepository([existing]));

    const result = await service.resolveBeforeCreate({ displayName: "Luis García", source: "admin", phones: [{ phone: "222" }] });
  ...
  it("returns no_match when no identity evidence exists", async () => {
    ...
    const result = await service.resolveBeforeCreate({ displayName: "Nuevo Paciente", source: "admin", phones: [{ phone: "333" }] });
