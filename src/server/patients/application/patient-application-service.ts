...
export class PatientApplicationService implements PatientApplicationServiceContract {
  ...
  async createPatient(command: CreatePatientApplicationCommand): Promise<PatientApplicationResult<Patient>> {
    const patient = await this.patientPersistencePort.createPatient(mapCreatePatientCommandToDomainInput(command));
  ...
  async updatePatient(id: PatientId, command: UpdatePatientApplicationCommand): Promise<PatientApplicationResult<Patient>> {
    ...
    const patient = await this.patientPersistencePort.updatePatient(id, mapUpdatePatientCommandToDomainInput(command));
    return { ok: true, patient };
  }

  async searchPatientsByIdentity(command: PatientIdentitySearchCommand): Promise<PatientSearchResult> {
    const patients = await this.patientPersistencePort.searchPatientsByIdentity(mapIdentitySearchCommandToPortSearch(command));
    return {
      ok: true,
      patients,
...
export function createPatientApplicationService(patientPersistencePort: PatientPersistencePort): PatientApplicationService {
