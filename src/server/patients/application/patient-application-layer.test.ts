...
import type { PatientPersistencePort, PatientIdentitySearch } from "../domain/patient-persistence-port";
...
import { createPatientUseCase, searchPatientsByIdentityUseCase, updatePatientUseCase } from "./patient-use-cases";

function createInMemoryPatientPort(seed: Patient[] = []): PatientPersistencePort & { searches: PatientIdentitySearch[] } {
  ...
  return {
    ...
    async createPatient(input: CreatePatientInput) {
      const patient = createPatientEntity(input, { id: input.id ?? `patient_${patients.size + 1}`, now: "2026-06-24T00:00:00.000Z" });
    ...
    async searchPatientsByIdentity(search: PatientIdentitySearch) {
      ...
      return Array.from(patients.values()).filter((patient) => {
        if (search.email && patient.emails.some((email) => email.normalizedEmail === search.email)) return true;
        if (search.phone && patient.phones.some((phone) => phone.normalizedPhone === search.phone)) return true;
...
describe("71.5.2 Patient Application Layer", () => {
  ...
  it("gets an existing patient and throws an application error for missing patients", async () => {
    const existing = createPatientEntity({ id: "patient_existing", displayName: "Existing", source: "admin", phones: [{ phone: "555" }] });
    const service = createPatientApplicationService(createInMemoryPatientPort([existing]));

    await expect(service.getPatientById("patient_existing")).resolves.toMatchObject({ ok: true, patient: existing });
    await expect(service.getPatientById("missing")).rejects.toBeInstanceOf(PatientApplicationLayerNotFoundError);
  });

  it("updates patients only after confirming the patient exists", async () => {
    const existing = createPatientEntity({ id: "patient_update", displayName: "Before", source: "admin", phones: [{ phone: "555" }] });
    ...
    await expect(updatePatientUseCase(port, "missing", { displayName: "Nope" })).rejects.toBeInstanceOf(PatientApplicationLayerNotFoundError);
  });

  it("normalizes identity searches before delegating to the port", async () => {
    const patient = createPatientEntity({ id: "patient_identity", displayName: "Lucia Gomez", source: "admin", phones: [{ phone: "+507 6000-0000" }] });
    const port = createInMemoryPatientPort([patient]);

    const result = await searchPatientsByIdentityUseCase(port, { normalizedName: "  Lucía   Gómez  ", phone: "+507 6000-0000" });
  ...
  it("requires duplicate candidates to stay in manual review instead of automated merge", async () => {
    const one = createPatientEntity({ id: "patient_dup_1", displayName: "Sam", source: "admin", phones: [{ phone: "555" }] });
    const two = createPatientEntity({ id: "patient_dup_2", displayName: "Sam", source: "admin", phones: [{ phone: "555" }] });
    const result = await createPatientApplicationService(createInMemoryPatientPort([one, two])).searchPatientsByIdentity({ phone: "555" });
  ...
  it("exposes use-case functions without binding to adapters or providers", async () => {
    const port = createInMemoryPatientPort();
    const result = await createPatientUseCase(port, { displayName: "Use Case", source: "admin", phones: [{ phone: "555" }] });
  ...
  it("maps identity search values deterministically", () => {
    expect(mapIdentitySearchCommandToPortSearch({ email: " TEST@Example.COM ", identifierValue: " cid 123 " })).toEqual({
