...
import { normalizeEmail, normalizeIdentifier, normalizeName, normalizePhone } from "../domain/patient.value-objects";
...
export function mapCreatePatientCommandToDomainInput(command: CreatePatientApplicationCommand): CreatePatientInput {
  const { metadata: _metadata, ...domainInput } = command;
  return domainInput;
}

export function mapUpdatePatientCommandToDomainInput(command: UpdatePatientApplicationCommand): UpdatePatientInput {
  const { metadata: _metadata, ...domainInput } = command;
  return domainInput;
}

export function mapIdentitySearchCommandToPortSearch(command: PatientIdentitySearchCommand): PatientIdentitySearch {
  return {
    ...
    identifierValue: command.identifierValue ? normalizeIdentifier(command.identifierValue) : undefined,
