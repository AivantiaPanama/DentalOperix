...
describe("61.2-06C AssistantAppointmentWorkflowCard", () => {
  ...
  it("creates an appointment request, checks provider availability, and confirms when available", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, appointment: { id: "appt_001", status: "requested" } }), {
          status: 201,
    ...
    fireEvent.change(screen.getByLabelText("Proveedor de la cita"), { target: { value: "provider_a" } });
    fireEvent.click(screen.getByRole("button", { name: "Crear solicitud de cita" }));

    expect(await screen.findByText("Solicitud de cita creada para revisión de disponibilidad.")).toBeDefined();
    ...
    expect(await screen.findByText("Cita confirmada con disponibilidad por proveedor.")).toBeDefined();
    expect(screen.getByText("confirmed")).toBeDefined();
    expect(fetchMock).toHaveBeenCalledWith("/api/appointments/request", expect.objectContaining({ method: "POST" }));
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/appointments/check-availability",
      expect.objectContaining({ method: "POST" }),
    );
    expect(fetchMock).toHaveBeenCalledWith("/api/appointments/confirm", expect.objectContaining({ method: "POST" }));
  ...
  it("routes unavailable requests to assistant review instead of hard failing", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, appointment: { id: "appt_002", status: "requested" } }), {
          status: 201,
          headers: { "Content-Type": "application/json" },
        }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ success: true, available: false, conflicts: [{ id: "appt_existing" }] }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }),
      )
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ success: true, appointment: { id: "appt_002", status: "needs_assistant_review" } }),
          { status: 200, headers: { "Content-Type": "application/json" } },
        ),
      );
    vi.stubGlobal("fetch", fetchMock);
    ...
    fireEvent.click(screen.getByRole("button", { name: "Verificar disponibilidad" }));
    expect(await screen.findByText("Proveedor no disponible; envía la solicitud a revisión.")).toBeDefined();
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/appointments/mark-review", expect.objectContaining({ method: "POST" }));
  });
});
