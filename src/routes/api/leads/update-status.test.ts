...
function request(body: unknown, headers: Record<string, string> = { "x-api-key": "test-key" }) {
  return new Request("http://localhost/api/leads/update-status", {
    method: "POST",
...
describe("/api/leads/update-status", () => {
  beforeEach(async () => {
    vi.resetAllMocks();
    ...
    const routeModule = await import("./update-status");
    POST = routeModule.POST;
  });

  it("returns 401 when neither session nor internal key is present", async () => {
    const response = await POST(request({ leadId: "lead-1", status: "contactado" }, {}));

    expect(response.status).toBe(401);
    expect(updateLead).not.toHaveBeenCalled();
  });

  it("updates only Lead status through the certified persistence adapter", async () => {
    updateLead.mockResolvedValue(undefined);

    const response = await POST(request({ leadId: "lead-1", status: "contactado" }));

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ success: true, leadId: "lead-1", status: "contactado" });
    expect(updateLead).toHaveBeenCalledWith("lead-1", { status: "contactado" });
    expect(appendLead).not.toHaveBeenCalled();
  });

  it("rejects invalid status values", async () => {
    const response = await POST(request({ leadId: "lead-1", status: "invalid" }));
  ...
  it("rejects missing leadId", async () => {
    const response = await POST(request({ status: "seguimiento" }));
  ...
  it("rejects extra fields to avoid widening the write contract", async () => {
    const response = await POST(
      request({ leadId: "lead-1", status: "seguimiento", notes: "No permitido" }),
    );
  ...
  it("returns 500 when the certified adapter fails", async () => {
    ...
    const response = await POST(request({ leadId: "missing", status: "seguimiento" }));
