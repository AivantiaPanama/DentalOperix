...
export async function fetchExecutiveAnalytics(period: string | null = null): Promise<ExecutiveAnalyticsApiResponse> {
  ...
  const payload = (await response.json()) as ExecutiveAnalyticsApiResponse | { success: false; error?: string };
  if (!payload.success) {
