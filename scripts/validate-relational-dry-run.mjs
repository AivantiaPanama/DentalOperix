...
if (persistenceMode !== 'google-sheet') {
  fail('Refusing dry-run unless LEADS_PERSISTENCE_MODE=google-sheet. Dry-run must not activate cutover.');
}

if (cutoverApproved) {
  fail('Refusing dry-run when RELATIONAL_CUTOVER_APPROVED=true. This validation is pre-cutover only.');
}

if (!dryRunApproved || !dryRunEnabled) {
  fail('RELATIONAL_DRY_RUN_APPROVED=true and RELATIONAL_DRY_RUN_ENABLED=true are required for this explicit dry-run.');
}

if (!schemaDeploymentApproved || !schemaDeploymentEnabled) {
  fail('Schema deployment flags must remain true after 57.7-C so the schema validation context is explicit.');
}
...
try {
  ...
  const readResult = await client.query('SELECT id, status, email_sent FROM leads WHERE id = $1', [id]);
  if (readResult.rowCount !== 1) {
    throw new Error('Dry-run inserted lead was not readable inside the transaction.');
  }

  await client.query('UPDATE leads SET status = $2, email_sent = $3 WHERE id = $1', [id, 'agendada', true]);

  const updateResult = await client.query('SELECT id, status, email_sent FROM leads WHERE id = $1', [id]);
  ...
  const auditResult = await client.query('SELECT id, migration_status FROM lead_persistence_migration_audit WHERE id = $1', [auditId]);
  ...
  console.log('Operations validated: INSERT lead, SELECT lead, UPDATE lead, INSERT audit, ROLLBACK cleanup');
