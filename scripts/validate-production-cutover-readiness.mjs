...
console.log(`Google Sheet rollback reference configured: ${googleSheetId && googleSheetName ? 'yes' : 'no'}`);
...
if (!runtimeActivationApproved) {
  fail('RELATIONAL_RUNTIME_ACTIVATION_APPROVED=true is required for production cutover readiness validation.');
}

if (!readinessApproved) {
  fail('RELATIONAL_PRODUCTION_CUTOVER_READINESS_APPROVED=true is required to run this controlled production readiness validation.');
}

if (!googleSheetId || !googleSheetName) {
  fail('GOOGLE_SHEET_ID and GOOGLE_SHEET_NAME are required as rollback/current-persistence references before production cutover.');
}
...
try {
  ...
  const auditCount = await client.query('SELECT COUNT(*)::int AS count FROM lead_persistence_migration_audit');
  const dbIdentity = await client.query('SELECT current_database() AS database_name, current_user AS database_user');
  ...
  console.log(`Database identity: ${dbIdentity.rows[0]?.database_name ?? 'unknown'} / ${dbIdentity.rows[0]?.database_user ?? 'unknown'}`);
