...
const cutoverApproved = process.env.RELATIONAL_CUTOVER_APPROVED || process.env.CUTOVER_APPROVED || '';
...
if (persistenceMode !== 'google-sheet') errors.push('LEADS_PERSISTENCE_MODE must remain google-sheet for 57.7-B connectivity validation.');
if (cutoverApproved !== 'false') errors.push('RELATIONAL_CUTOVER_APPROVED must remain false for 57.7-B connectivity validation.');
if (connectivityEnabled !== 'true') errors.push('RELATIONAL_CONNECTIVITY_TEST_ENABLED must be true to run this connectivity validation.');
