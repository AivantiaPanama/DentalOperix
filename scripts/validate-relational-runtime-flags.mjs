...
if (persistenceMode !== "relational-db") {
  fail(
    "LEADS_PERSISTENCE_MODE must be relational-db for controlled runtime activation validation.",
  );
}

if (!cutoverApproved) {
  fail(
    "RELATIONAL_CUTOVER_APPROVED=true is required for controlled runtime activation validation.",
  );
}

if (!runtimeActivationApproved) {
  fail(
    "RELATIONAL_RUNTIME_ACTIVATION_APPROVED=true is required for controlled runtime activation validation.",
  );
}
