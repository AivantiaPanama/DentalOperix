...
if (!postCutoverApproved) {
  fail('RELATIONAL_POST_CUTOVER_VALIDATION_APPROVED=true is required to run this controlled post-cutover validation.');
}
...
try {
  ...
  const inserted = await client.query('SELECT id, status, email_sent FROM leads WHERE id = $1', [syntheticLeadId]);
  ...
  const updated = await client.query('SELECT id, status, email_sent FROM leads WHERE id = $1', [syntheticLeadId]);
  ...
  const residual = await client.query('SELECT COUNT(*)::int AS count FROM leads WHERE id = $1', [syntheticLeadId]);
