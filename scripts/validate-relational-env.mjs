...
for (const key of ['GOOGLE_SHEET_ID', 'GOOGLE_SHEET_NAME']) {
  if (!env[key]) warnings.push(`${key} is missing; rollback/current-persistence validation will be incomplete.`);
}

if (mode === 'relational-db') {
  if (!databaseUrl) errors.push('Relational mode requires DATABASE_URL or POSTGRES_URL.');
  if (!cutoverApproved) {
    errors.push('LEADS_PERSISTENCE_MODE=relational-db requires RELATIONAL_CUTOVER_APPROVED=true for governance validation.');
  }
}
...
if (!connectivityEnabled) {
  warnings.push('Relational connectivity test disabled. This validates configuration shape only, not database reachability.');
}
