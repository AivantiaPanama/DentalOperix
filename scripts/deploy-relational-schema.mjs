...
const sqlPath = process.argv[3] ?? 'docs/architecture/sql/57_7_C_supabase_leads_schema_deployment.sql';
...
if (persistenceMode !== 'google-sheet') {
  fail('Refusing schema deployment unless LEADS_PERSISTENCE_MODE=google-sheet. Schema deployment is not cutover.');
}

if (cutoverApproved) {
  fail('Refusing schema deployment when RELATIONAL_CUTOVER_APPROVED=true. Use cutover runbook for activation steps.');
}

if (!schemaDeploymentApproved || !schemaDeploymentEnabled) {
  fail('Schema deployment requires RELATIONAL_SCHEMA_DEPLOYMENT_APPROVED=true and RELATIONAL_SCHEMA_DEPLOYMENT_ENABLED=true.');
}
